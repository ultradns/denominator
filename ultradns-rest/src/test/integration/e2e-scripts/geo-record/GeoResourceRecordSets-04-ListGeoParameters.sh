...
listGeoRecordType="geo -z $zoneName types"
listAvailableRegions="geo -z $zoneName regions"
zoneList="zone list -n $zoneName"
zoneDelete="zone delete -i "$zoneName

# E2E Script
echo "----------------------------------------------------------------------------------------"
echo "Test Scenario: Geo Resource Record Sets - 04 - List Geo Params"
echo "----------------------------------------------------------------------------------------"
echo "STARTING TESTS"
echo " "
...
# List available geo record types for zone
hitService "$listGeoRecordType" > $operation_logfile 2> $operation_logfile
validate "CNAME" "List available geo record types for zone"
cat $operation_logfile >> $api_calls_logfile
